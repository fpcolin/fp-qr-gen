# Building and releasing the QR Code Generator
...
fp-qr-gen/
├─ src/
│  ├─ fp_qr_gen.pyw       main application (.pyw = no console)
│  ├─ updater.py          update check (must stay .py - see below)
│  ├─ fp.ico              app + installer icon
│  └─ fp_logo.png         default embedded logo
├─ build/
│  ├─ fp_qr_gen.spec      PyInstaller config
...
`fp_qr_gen.pyw` uses the `.pyw` extension so that double-clicking it on Windows
...
You get `dist\installer\FPQRGenerator-<version>-setup.exe`.
...
```
python build\publish.py dist\installer\FPQRGenerator-2.1.0-setup.exe 2.1.0 ^
    --base-url https://github.com/fpcolin/fp-qr-gen/releases/download/v2.1.0 ^
...
server never blocks someone from generating a QR code.
...
The workflow checks the tag against `VERSION` in `fp_qr_gen.pyw` and fails if
they disagree, then rewrites `version_info.txt` and passes `/DAppVersion` to
Inno Setup. That makes `fp_qr_gen.pyw` the single source of truth and removes
...
`pkg_resources` runtime hook altogether. Nothing in this app, qrcode, or Pillow
imports `pkg_resources`, so this is safe.
...
```yaml
      - uses: azure/artifact-signing-action@v1
        with:
          ...
          files-folder: ${{ github.workspace }}\dist\FPQRGenerator
...
is `FPQRGenerator-2.1.0-setup.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART`.
