...
def _urlopen(url: str, timeout: int):
    ...
    request = urllib.request.Request(url, headers={'User-Agent': 'FPQRGen-Updater'})
...
def check(current_version: str) -> dict | None:
    ...
    never stop someone generating a QR code.
    """
    try:
...
def download(manifest: dict, progress=None) -> Path:
    ...
    target = Path(tempfile.gettempdir()) / f'fp_qr_gen_setup_{manifest["version"]}.exe'
